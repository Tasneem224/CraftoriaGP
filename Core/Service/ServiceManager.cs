...
namespace Service
{
    public class ServiceManager(PaymentServiceFactory _paymentFactory,IUserInteractionService userInteractionService,IHttpContextAccessor _httpContextAccessor,ITranslationService _translationService,ICartRepository cartRepository,ICartService cartService,IAccountService accountService,ICacheRepository cacheRepository,ICacheService cacheService,IUnitOfWork unitOfWork,UserManager<ApplicationUser> _userManager, IConfiguration _configuration, ICloudinaryService _cloudinaryService, IEmailService _emailService,IEmailVerificationCodeRepository _emailVerificationrRepo,IMapper _mapper,ICartRepository _cacheRepository) : IServiceManager
    {
        ...
        private readonly Lazy<IProductService> _LazyProductService = new Lazy<IProductService>(() => new ProductService(userInteractionService, _translationService, _httpContextAccessor, _userManager,_cloudinaryService,unitOfWork));
